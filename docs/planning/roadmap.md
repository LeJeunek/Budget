...
## Immediate next step
**Phase 4b's Reports and Notifications v2 specs have passed CTO review — milestone 3 is cleared to begin.** `docs/product/reports.md` and `docs/product/notifications-v2.md` are both final. The 2026-07-27 CTO resolution pass (in the Phase 4b section above) found no cross-doc conflicts between the two new specs, confirmed both stayed inside the kickoff pass's binding constraints (zero new `lib/ai/` call sites; deterministic, threshold-only Large Purchase/Low Balance triggers; verbatim-only `MonthlySummary.narrative` reuse, omitted when null), verified their citations of already-shipped product docs (`ai-features.md`, `financial-goals.md`, `calendar-and-notifications.md`, `transactions.md`, `accounts.md`, `dashboard-overview.md`, `analytics.md`) against those docs' actual text, and ratified the Product Owner's decision to drop Weekly Summary from this phase's scope. No Product Owner follow-up dispatch is required.

Dispatch the **Solution Architect + Database Architect combined pass** (milestone 3): resolve the PDF generation library and email delivery provider choices within the constraints already stated in the Phase 4b section above; extend the `Notification` model/enum for the four new trigger types (Goal Achieved, Large Purchase, Low Balance, Monthly Summary); design the email-sending module boundary as its own independent concern, kept separate from `lib/ai/` and from the PDF renderer. No AI Engineer participation is needed for this pass, per the binding constraints already set.

4c (Calendar v2, Customization, Admin) remains blocked until 4b's own full review gate (milestone 6) passes.
