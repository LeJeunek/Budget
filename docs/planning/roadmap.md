...
## Immediate next step
**Phase 4b (Reports & Notifications v2) is ready for the Product Owner.** 4a's review gate has passed (`docs/release/phase-4a-notes.md`, APPROVE), and this document's own Phase 4b CTO kickoff pass (2026-07-27, above) has confirmed scope and set the binding reuse/integration constraints implementers must follow. Dispatch a **Product Owner spec pass for Reports and Notifications v2, in parallel** (they are independent domains, per this phase's own milestone 1). The Product Owner must:
- Resolve the "weekly vs. monthly summary" notification-trigger ambiguity flagged above (a real scope decision — keep both as distinct, differently-shaped features, or narrow to monthly-only) before the spec is considered final.
- Write Reports' spec so the Monthly report's narrative section is explicitly specified as a reuse of the existing `MonthlySummary.narrative` (with the "narrative omitted, numeric sections still render" degraded-state behavior), not a newly-generated one.
- Write Notifications v2's spec so "large purchase" and "low balance" are explicitly specified as deterministic, threshold-based triggers, independent of Spending Insights.

Do not let the Product Owner or an implementing agent guess at the PDF library or email provider — both remain flagged for the Solution Architect + Database Architect architecture pass (milestone 3), within the constraints this document states above. 4c (Calendar v2, Customization, Admin) does not get architected until 4b's review gate passes.
