...
**Phase 4b note on this convention:** neither Reports nor Notifications v2 introduces a client-instant-recompute requirement — report generation is a one-shot fetch-and-download per click (never a per-keystroke recompute), and every notification trigger evaluation happens server-side (a lazy on-read check or a cron pass), never in the browser. No file in either module earns the feature-root/`-math.ts` treatment; see `phase-4b-technical-design.md` §3/§6 for the full module layouts, both entirely under `server/`.
...
**New Phase 4b example — Reports: synchronous, on-demand, zero-persistence generation:**
```
User requests a report (Client Component, features/reports/components/report-download-button.tsx)
  → GET /api/reports?type=monthly&month=2026-06 (session-authenticated Route Handler)
    → features/reports/server/service.ts.generateReport(userId, request)
      → validation.ts validates the request shape (rejects a not-yet-started future period here,
        per reports.md's Edge Case — never generates an empty report for one)
      → period.ts resolves the exact date range
      → features/reports/server/data/monthly-report-data.ts.assembleMonthlyReportData(userId, month)
        → calls ONLY existing, already-reviewed read functions across Dashboard/Budgeting (per
          reports.md's binding constraint 2) — zero new aggregation logic, zero lib/ai/ import
      → features/reports/server/render.ts.renderReportPdf("MONTHLY", data)
        → @react-pdf/renderer's renderToBuffer(<MonthlyReportTemplate data={data} />)
  ← raw application/pdf bytes, Content-Disposition: attachment (success path — a deliberate,
    narrow exception to ApiResult<T>, per phase-4b-technical-design.md §3) | ApiResult<never>
    JSON error (failure path, distinguishable by Content-Type before any download is attempted)

Nothing is persisted at any point in this flow — no Report row, no report ID, nothing to fetch
by a second request. See phase-4b-technical-design.md §2's "no stored artifact to leak" note.
```

**New Phase 4b example — Notifications v2: the lazy on-read pattern, extended, plus a new cron for offline reach:**
```
Path A — a user's own request (unchanged mechanism from Notifications v1, now evaluating six
trigger types instead of two)
  GET /api/notifications (session-authenticated)
    → features/notifications/server/service.ts.ensureNotifications(userId)
      → triggers/budget-bill-triggers.ts (existing, unchanged), triggers/goal-achieved-trigger.ts,
        triggers/large-purchase-trigger.ts, triggers/low-balance-trigger.ts,
        triggers/monthly-summary-trigger.ts (all NEW) — each independently evaluates its own
        domain's live data and upserts Notification rows, deduped by a DB unique constraint or an
        atomic conditional update (never read-then-write — the Finding-6b lesson reused from 4a)
      → for every row newly created this pass: email-dispatch.ts checks that user's
        NotificationPreference for this row's type; if emailEnabled, calls
        lib/email/send-notification-email.ts (never throws — a failure only ever affects
        Notification.emailSentAt/emailSendError, never the in-app row itself, per AC7)
  ← ApiResult<{ items: Notification[]; unreadCount: number }>

Path B — the new cron (reaches users who never open the app, per email's own stated purpose)
  POST /api/cron/evaluate-notifications (shared-secret authenticated)
    → loops every user, calling the SAME ensureNotifications(userId) as Path A — evaluation logic
      is written exactly once; both paths are safe to race each other, since every dedup mechanism
      is a database-level guarantee, not an in-process flag
  ← { processed: number; emailsSent: number } plain JSON
```
...
| `features/<domain>/server/` | Backend Engineer (AI Engineer for the AI-owned files within it, per the sibling-file convention above) | `lib/db.ts`, `lib/auth.ts`, `lib/uploadthing.ts` (Transactions only), `lib/recurrence.ts` (Bills, Recurring Income), `lib/transaction-link-guard.ts` (Bills, Recurring Income), `lib/merchant-normalization.ts` (Analytics), **`lib/ai/`'s exported functions/types only, never a provider SDK directly (Transactions, Budgeting, Dashboard, Analytics, Financial Health Score — Phase 4a)**, **`lib/email/`'s exported functions/types only, never the `resend` package directly (Notifications — Phase 4b)**, **`@react-pdf/renderer` isolated to `features/reports/server/render.ts` only (Phase 4b)**, other domains' server code only via explicit, individually-exported service calls (not direct Prisma reach-through) | `app/`, `components/` |
...
| `lib/` | Solution Architect + Database Architect (db client), Backend Engineer (auth helpers, UploadThing SDK singleton, recurrence math, transaction-link guard, merchant normalization, email sending — Phase 4b) | — | — |
| `lib/ai/` | AI Engineer | `@ai-sdk/google` / the `ai` package (isolated to `client.ts` only, revised from an initial `@ai-sdk/anthropic` decision — see `ai-features-design.md`'s provider-swap addendum), nothing feature-specific | `app/`, `components/`, any `features/*` — **this is the one direction that must never be crossed**: features import `lib/ai/`, `lib/ai/` never imports a feature back. This is what keeps the module graph acyclic despite five features depending on it (see the Phase 4a dependency graph below). **Confirmed by Phase 4b: neither `features/reports/` nor `features/notifications/` ever imports `lib/ai/` at all** — zero new call sites, per Risk #19 and `phase-4b-technical-design.md` §8. |
| `lib/email/` (Phase 4b) | Backend Engineer | `resend` / `@react-email/*` (isolated to `client.ts`/`templates/` only), nothing feature-specific | `app/`, `components/`, any `features/*`, `lib/ai/` — the same one-directional "pure fan-in leaf" rule `lib/ai/` already follows, applied here for a second third-party dependency |

This prevents circular dependencies: UI components never know about features; features never import from `app/`; only `app/` composes both; `lib/ai/` is a pure fan-in leaf exactly like `lib/recurrence.ts`, never a fan-out source back into any feature; and, as of Phase 4b, `lib/email/` is a second, independent pure fan-in leaf alongside it.
...
## Reusable utilities added in Phase 4b

- **`lib/email/`** (NEW MODULE) — `client.ts` (the only file touching the `resend` package / reading `RESEND_API_KEY`), `send-notification-email.ts` (the one reusable, never-throwing "render + send" call every notification trigger uses), `templates/` (one React Email component per trigger type), `unsubscribe-token.ts` (signed, single-purpose `(userId, type)` tokens for the one-click unsubscribe link). Full behavior and reasoning: `phase-4b-technical-design.md` §4–§5. This document only records *where it sits in the module map* (above) and *that nothing else may talk to the email provider directly*.
- **`features/reports/server/render.ts`** — the one function that calls `@react-pdf/renderer` directly; every one of the six report templates under `features/reports/server/pdf/templates/` is reached only through it. Full behavior and reasoning: `phase-4b-technical-design.md` §2–§3.
...
This is the first model in this schema whose entire purpose is "durably record an exclusion decision about a *computed, never-persisted* concept" (contrast with `Notification`, which dismisses an already-persisted row, or every `archivedAt` column, which soft-deletes an already-persisted entity) — flagged explicitly as a new, small pattern, the same way `NetWorthSnapshot` was flagged in Phase 3a as "the first not-request-triggered write path." **(Phase 4b) `FinancialGoal.completionNotifiedAt` is this pattern's mirror image — a durable "already notified" latch over a different computed, never-persisted concept (goal completion) — see `phase-4b-technical-design.md` §7.3.**
...
- **(Phase 3a) The Net Worth Snapshot job is the first scheduled/cron surface in this codebase** — unchanged by Phase 3b; Phase 4a adds three more cron surfaces (categorization, monthly summary, health-score snapshot), all following the exact same shared-secret/no-session/plain-JSON exception this job established first. **Phase 4b adds a fifth** (`evaluate-notifications`) — see the Phase 4b section below.
...
Per `roadmap.md`'s Phase 4b section, `docs/product/reports.md`, `docs/product/notifications-v2.md`, and **[docs/architecture/phase-4b-technical-design.md](phase-4b-technical-design.md)** (this Architect's own full technical design — the PDF library decision, the email provider decision, per-report-type data assembly, and the complete `Notification` model/enum extension specification for the Database Architect). This section records only the module-map placement and dependency-graph additions that document left to this file, mirroring exactly how the Phase 4a section above handles the identical split with `ai-features-design.md`.

### Two new modules, one extended module — none of them touching `lib/ai/`

- **`features/reports/`** — NEW feature module. A pure "leaf" consumer, structurally identical to Financial Goals/Financial Health Score: it reads from Dashboard, Analytics, Debt, Investments, Budgeting, and Recurring Income, and nothing ever reads from it. See `phase-4b-technical-design.md` §2–§3 for the PDF library decision and the full module layout.
- **`lib/email/`** — NEW cross-feature infrastructure module, placed in `lib/` for the same reason `lib/ai/` is: multiple feature call sites need it (every one of the six notification trigger types), and it is framework-agnostic aside from one isolated third-party import (`client.ts`). See `phase-4b-technical-design.md` §4–§5 for the email provider decision, the module layout, and the unsubscribe/preference design.
- **`features/notifications/`** — existing Phase 2 module, extended. Gains four new trigger files (`triggers/goal-achieved-trigger.ts`, `large-purchase-trigger.ts`, `low-balance-trigger.ts`, `monthly-summary-trigger.ts`), a shared `email-dispatch.ts` step, and a `preferences.ts` file for the new preferences/threshold-settings read/write surface. See `phase-4b-technical-design.md` §6 for the full evaluation architecture (the lazy-on-read pattern extended, plus the new `evaluate-notifications` cron route) and §7 for the schema it depends on.

### Module boundary table additions

| Folder | Owner | May import from | Must NOT import from |
|---|---|---|---|
| `features/reports/server/` | Backend Engineer | `lib/db.ts`, `lib/auth.ts`, `@react-pdf/renderer` (isolated to `render.ts` only), other domains' server code only via explicit, individually-exported service calls (Dashboard, Analytics, Debt, Investments, Budgeting, Recurring Income) | `app/`, `components/`, `lib/ai/`, `lib/email/` |
| `features/notifications/server/` | Backend Engineer | `lib/db.ts`, `lib/auth.ts`, `lib/email/`'s exported functions/types only, never the `resend` package directly; other domains' server code (Budgeting, Bills, Financial Goals, Transactions, Accounts, Dashboard) only via explicit service calls | `app/`, `components/`, `lib/ai/` |
| `lib/email/` | Backend Engineer | `resend` package / `@react-email/*` (isolated to `client.ts`/`templates/` only), nothing feature-specific | `app/`, `components/`, any `features/*`, `lib/ai/` — same one-directional "pure fan-in leaf" rule `lib/ai/` and `lib/recurrence.ts` already follow |

### Dependency graph additions

```
Dashboard, Analytics, Debt, Investments, Budgeting, Recurring Income    (existing — each read
        │        │        │        │          │            │            live, never re-derived)
        └────┬───┴────┬───┴────┬───┴─────┬────┴──────┬─────┘
             ↓         ↓        ↓         ↓           ↓
                  features/reports/server/data/*.ts     (NEW leaf module — six report-type data
                             │                            assemblers, ZERO AI dependency, ZERO
                             ↓                            persistence of their own)
                  features/reports/server/pdf/*.tsx  →  @react-pdf/renderer (isolated to render.ts)

Budgeting, Bills, Financial Goals, Transactions, Accounts, Dashboard    (existing — each read
        │        │         │              │           │        │        live, never re-derived)
        └────┬───┴────┬────┴───────┬──────┴─────┬─────┘
             ↓         ↓            ↓            ↓
              features/notifications/server/triggers/*.ts   (six trigger files — zero AI
                             │                                dependency, per Risk #19/#20)
                             ↓
              features/notifications/server/email-dispatch.ts  ───────▶  lib/email/  ───▶ resend
                                                                            (isolated to client.ts)

lib/ai/ never imports back into any feature (unchanged) — and, new this phase, NEITHER
features/reports/ NOR features/notifications/ ever imports from lib/ai/ at all (Risk #19),
confirmed by an ESLint no-restricted-imports rule scoped to both directories (see
phase-4b-technical-design.md §8).
```

### Risks / scalability notes (Phase 4b)

- **(Phase 4b) `features/reports/` is this codebase's first feature whose entire purpose is producing a downloadable artifact rather than an in-app view** — resolved as a zero-persistence, synchronous-generation design specifically because it removes an entire class of cross-user-leakage surface (no stored report, no report ID, nothing to fetch by guessing an identifier) rather than needing to defend one. See `phase-4b-technical-design.md` §2's full reasoning.
