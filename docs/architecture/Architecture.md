...
**Phase 3b status note:** this pass assumes Phase 3a's schema (as actually built and documented in `docs/database/er-diagram.md`'s Phase 3a design notes) is settled and live — the Account-linkage question above resolved to the hybrid, optional-link shape (Option C) and is no longer an open variable. Phase 3b hands the Database Architect two new schema requirements — `FinancialGoal` (plus a small join table for its Account-subset measurement basis) and `DismissedSubscriptionMerchant` — both specified in full below, under "Phase 3b — Analytics, Net Worth History, Financial Goals," following the same "flag the requirement precisely, Database Architect makes the final call" pattern Phase 3a established for Debt/Investments.

## Guiding pattern: feature-first modules under App Router

Each business domain (accounts, transactions, dashboard, categories, budgeting, goals, bills, notifications, debt, investments, recurring-income, analytics, financial-goals, and later AI) is a **feature module**: its own folder under `features/`, containing everything specific to that domain — server logic, types, validation schemas, hooks — with `app/` staying thin (routing + composition only) and `components/` staying generic (no domain knowledge).
...
- **Server Actions** (`features/<domain>/server/actions.ts`) for mutations. Phase 3a adds: debt create/update/archive/unarchive and optional link/unlink-to-account; holding create/update/close and dividend logging; income stream create/update/archive/unarchive and occurrence mark-received/link/unmark. **Phase 3b adds:** `features/analytics/server/actions.ts`'s single mutation, `dismissSubscriptionCandidate`; `features/financial-goals/server/actions.ts`'s `createFinancialGoal`, `updateFinancialGoal`, `archiveFinancialGoal`, `unarchiveFinancialGoal` (the familiar archive/unarchive CRUD shape, with **no** contribution/progress-logging action of any kind — see the Financial Goals section below for why that omission is the feature's defining, deliberate property, not a gap).
- **Route Handlers** (`app/api/<domain>/route.ts`) only where a true HTTP endpoint is needed. Phase 3a adds `app/api/debts/route.ts`, `app/api/investments/route.ts`, `app/api/income/route.ts`, and `app/api/cron/net-worth-snapshot/route.ts`. **Phase 3b adds exactly one:** `app/api/dashboard/net-worth-history/route.ts` — see "Net Worth History chart" below for why this is the *first* Dashboard Route Handler ever needed (Phase 1/2's Dashboard functions were all Server-Component-direct-call-only, per Architecture.md's own note that nothing client-side needed to refetch them independently of a full page load — the range selector changes that). Analytics and Financial Goals need **no** new Route Handlers at all — see their own sections below for why the existing searchParam-navigation and Server-Action-plus-`revalidatePath` patterns already cover everything either feature needs.
- **Client Components** are opt-in (`"use client"`) for interactive pieces. Phase 3a adds the snowball/avalanche strategy comparison, the extra-payment input, the allocation/growth charts, and the holding/income-stream forms. **Phase 3b adds:** the Net Worth History chart itself (`features/dashboard/components/net-worth-history-chart.tsx`, Recharts, with its own range-selector and breakdown-toggle local state — see below), and ordinary React Hook Form + Zod forms for Financial Goals' three goal-type creation flows (`features/financial-goals/components/*-goal-form.tsx`) and Subscription Cost Detection's dismiss button. Analytics' per-metric chart/table components (`features/analytics/components/*.tsx`) are **Server Components** wherever possible (they render a fully-resolved data shape with no further client interactivity beyond hover tooltips, which Recharts/shadcn components handle internally) — only the shared reporting-period selector control itself needs to be a small Client Component, since selecting a new period must trigger client-side navigation to the new searchParam value.
- TanStack Query is used client-side only where a genuine client-cache benefit exists (unchanged rule from Phase 1/2/3a). **Phase 3b adds exactly one new hook**, `features/dashboard/hooks/use-net-worth-history.ts` (fetches `GET /api/dashboard/net-worth-history?range=`, refetching when the user changes the range selector — the same "toggle triggers a refetch" shape as `use-debts.ts`'s `includeArchived` toggle, just keyed by a 4-value range enum instead of a boolean). Financial Goals gets **no** hook this phase: it has no client-side toggle-and-refetch need (its list page is a plain Server Component read, same shape as Savings Goals' own list page) and no client-instant-recompute need (unlike Debt). Analytics gets **no** hook either, for the same reason — its shared reporting-period control is a searchParam, not a client-side toggle needing a cache-refetch.
...
| `features/<domain>/server/` | Backend Engineer | `lib/db.ts`, `lib/auth.ts`, `lib/uploadthing.ts` (Transactions only), `lib/recurrence.ts` (Bills, Recurring Income — Phase 3a), `lib/transaction-link-guard.ts` (Bills, Recurring Income — Phase 3a), `lib/merchant-normalization.ts` (Analytics — Phase 3b), other domains' server code only via explicit, individually-exported service calls (not direct Prisma reach-through) | `app/`, `components/` |
...
- **(Phase 3a) The Net Worth Snapshot job is the first scheduled/cron surface in this codebase** — unchanged by Phase 3b; the chart is a pure read layer on top of the data this job has been accumulating since Phase 3a.
