...
|---|---|---|
| Dashboard, Budgeting, Recurring Income (`StatCard` consumers — re-verified directly for this correction: `dashboard-card-groups.tsx`, `budget-summary-cards.tsx`, `expected-upcoming-income-card.tsx` are the only three real `StatCard` call sites in `src/`, not the four this table originally claimed) | `value={formatCurrency(x, currency)}` | `value={<AnimatedNumber value={x} format={(n) => formatCurrency(n, currency)} />}` — `StatCard` itself unchanged beyond §2.2's type widening. |
...
| **Goals/Financial Goals** (`goal-card.tsx`, `financial-goal-card.tsx`, same plain-span shape as Accounts/Debt — a correction to this table, not to the product spec: neither file imports `StatCard`, both call `useFormatCurrency()` directly in plain markup) | `{formatCurrency(goal.currentProgress)} of {formatCurrency(goal.targetAmount)}` (and equivalent for Financial Goals) | Identical pattern to Accounts/Debt — `format={formatCurrency}` directly, no `StatCard` involvement. |
| **Bills** (`bill-list.tsx`, `upcoming-bills-list.tsx`, same plain-span shape — a correction to this table: no `StatCard` import anywhere in `features/bills/`) | `{formatCurrency(bill.expectedAmount)}` per row/list item | Identical pattern to Accounts/Debt — `format={formatCurrency}` directly. Per the product spec's own AC7 "headline, not row-level" gate, only a genuine per-page headline figure (not every bill row) is in scope here — a Frontend Lead per-surface judgment call, the same as Analytics' row below. |
